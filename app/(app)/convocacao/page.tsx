...
type Atleta = { id: string; nome: string; fotoUrl: string | null; turmaId: string | null }
...
export default function Convocacoes() {
  ...
  function toggleAtleta(id: string) { setAtletasSel(p => p.includes(id) ? p.filter(x => x !== id) : [...p, id]) }
  function selecionarTodos() { setAtletasSel(atletasSel.length === atletas.length ? [] : atletas.map(a => a.id)) }

  function salvar() {
    if (!form.titulo || !form.data || !form.horario) return alert('Título, data e horário obrigatórios.')
    if (atletasSel.length === 0) return alert('Selecione pelo menos um atleta.')
    startSave(async () => {
      await criarConvocacao(escolaId, form as Record<string, string>, atletasSel)
      setForm({ titulo: '', tipo: 'amistoso', data: '', horario: '', local: '', descricao: '' })
      setAtletasSel([]); setShowForm(false); carregar()
    })
  }
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ background: '#080C15', border: `1px solid ${T.border}`, borderRadius: 8, padding: 12 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                  ...
                  <button onClick={selecionarTodos} style={{ fontSize: 11, color: T.accent, fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer' }}>{atletasSel.length === atletas.length ? 'Desmarcar todos' : 'Todos'}</button>
                </div>
                ...
                <div style={{ maxHeight: 200, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 4 }}>
                  {filtrados.map(a => {
                    const sel = atletasSel.includes(a.id)
                    return (
                ...
                <p style={{ fontSize: 11, color: T.muted, marginTop: 8 }}>{atletasSel.length} selecionado(s)</p>
              </div>
          ...
          {convocacoes.map(c => {
            ...
            return (
              <div key={c.id} style={{ background: T.surface, border: `1px solid ${T.border}`, borderLeft: `3px solid ${cor}`, borderRadius: 8, padding: 16, marginBottom: 10 }}>
                ...
                <p style={{ fontSize: 11, color: T.muted, marginBottom: 4 }}><i className="ti ti-calendar" style={{ fontSize: 12, marginRight: 4 }} aria-hidden="true"></i>{new Date(c.data + 'T00:00:00').toLocaleDateString('pt-BR')} · {c.horario}</p>
                {c.local && <p style={{ fontSize: 11, color: T.muted, marginBottom: 10 }}><i className="ti ti-map-pin" style={{ fontSize: 12, marginRight: 4 }} aria-hidden="true"></i>{c.local}</p>}
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  {c.status === 'aberta' && <button onClick={() => { startSave(async () => { await encerrarConvocacao(c.id); carregar() }) }} style={{ background: T.border, color: T.muted, padding: '9px 12px', borderRadius: 8, fontSize: 12, border: 'none', cursor: 'pointer' }}>Encerrar</button>}
                  <button onClick={() => { if (!confirm('Excluir?')) return; startSave(async () => { await excluirConvocacao(c.id); carregar() }) }} style={{ background: 'rgba(255,68,68,0.08)', color: '#FF4444', padding: '9px 12px', borderRadius: 8, fontSize: 12, border: '1px solid rgba(255,68,68,0.2)', cursor: 'pointer' }}>
