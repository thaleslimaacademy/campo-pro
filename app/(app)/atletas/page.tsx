...
type Atleta = { id: string; nome: string; posicao: string | null; fotoUrl: string | null; bolsista: boolean | null; dataNascimento: string | null; turmaId: string | null }
...
export default function Atletas() {
  ...
  const subtitle = (a: Atleta) => [a.posicao, anoNasc(a.dataNascimento), a.turmaId ? turmaMap.get(a.turmaId) : null].filter(Boolean).join(' · ')
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, color: T.text, fontFamily: INTER, paddingBottom: 90 }}>
      ...
      <div style={{ padding: '8px 18px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        ...
        {filtrados.map(a => (
          <ListRow key={a.id} href={`/atletas/${a.id}`} fotoUrl={a.fotoUrl} initials={a.nome.slice(0, 2).toUpperCase()} title={a.nome} subtitle={subtitle(a) || 'Sem posição'} badge={a.bolsista ? 'Bolsista' : undefined} badgeColor={T.green} />
        ))}
      </div>
    </div>
  )
