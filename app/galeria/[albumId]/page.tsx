...
import { ShoppingCart, X, Check, Loader2, Copy, CreditCard } from 'lucide-react'
...
type Etapa = 'galeria' | 'form' | 'pix' | 'cartao' | 'sucesso'

export default function GaleriaAlbumPage({ params }: { params: { albumId: string } }) {
  ...
  useEffect(() => {
    listarFotosPublicas(albumId).then(d => setFotos(d as Foto[])).finally(() => setLoading(false))
  }, [albumId])
  ...
  const confirmarCompra = async () => {
    ...
    setEnviando(true)
    try {
      const r = await criarCompraPublica({
        compradorNome: nome,
        compradorTelefone: telefone,
        fotos: Array.from(selecionadas),
        metodoPagamento: metodo,
        parcelas,
      })
      setValorTotal(r.valor)
      if (metodo === 'PIX' && r.pixData) {
        setPixData(r.pixData)
        setEtapa('pix')
      } else if (r.creditCardUrl) {
        setCreditCardUrl(r.creditCardUrl)
        setEtapa('cartao')
      }
    } catch (e) {
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text }}>
      ...
      {etapa === 'galeria' && (
        <div style={{ padding: 16, maxWidth: 900, margin: '0 auto' }}>
          <p style={{ color: C.muted, fontSize: 12, marginBottom: 16 }}>
            Selecione as fotos que deseja comprar. As fotos originais sem marca d'água serão enviadas via WhatsApp após o pagamento.
          </p>
      ...
      {etapa === 'form' && (
        <div style={{ padding: 20, maxWidth: 480, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Campo label="WhatsApp * (as fotos serão enviadas aqui)">
              <input value={telefone} onChange={e => setTelefone(e.target.value)} placeholder="(34) 99999-9999" style={inp} />
            </Campo>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
            <button onClick={() => setEtapa('galeria')} style={{ flex: 1, background: 'transparent', color: C.muted, border: `1px solid ${C.border}`, borderRadius: 12, padding: '13px', cursor: 'pointer' }}>
              Voltar
            </button>
      ...
      {etapa === 'pix' && pixData && (
        <div style={{ padding: 20, maxWidth: 420, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 20 }}>Após o pagamento as fotos serão enviadas automaticamente no WhatsApp.</p>
      ...
      {etapa === 'cartao' && creditCardUrl && (
        <div style={{ padding: 20, maxWidth: 420, margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 24 }}>Clique no botão abaixo para pagar com cartão de crédito. Após a confirmação, as fotos serão enviadas no WhatsApp.</p>
