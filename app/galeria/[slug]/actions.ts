...
export async function criarCompraPublica(slug: string, p: {
  ...
  const dadosCobranca: Parameters<typeof criarCobrancaGenerica>[0] = {
    customer: cliente.id, billingType: p.metodoPagamento,
    dueDate, description: desc, value: valorTotal,
  }
  ...
  await supabaseAdmin.from('FotoCompra').insert({
    id: compraId, escolaId,
    compradorNome: p.compradorNome, compradorTelefone: telefone,
    fotos: p.fotos, valor: valorTotal, status: 'PENDENTE',
