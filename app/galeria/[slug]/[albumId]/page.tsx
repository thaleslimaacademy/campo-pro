...
export default function GaleriaAlbumPage({ params }: { params: Promise<{ slug: string; albumId: string }> }) {
  const { slug, albumId } = use(params)
  ...
  useEffect(() => {
    if (albumId) listarFotosPublicas(slug, albumId).then(d => setFotos(d as Foto[])).finally(() => setLoading(false))
  }, [slug, albumId])
  ...
  const confirmarCompra = async () => {
    ...
    setEnviando(true)
    try {
      const r = await criarCompraPublica(slug, { compradorNome: nome, compradorTelefone: telefone, fotos: Array.from(selecionadas), metodoPagamento: metodo, parcelas })
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text }}>
      ...
      {etapa === 'galeria' && (
        <div style={{ padding: 16, maxWidth: 900, margin: '0 auto' }}>
          <p style={{ color: C.muted, fontSize: 12, marginBottom: 16 }}>Selecione as fotos que deseja comprar. As originais sem marca d'água serão enviadas via WhatsApp após o pagamento.</p>
          {fotos.length === 0 ? (
