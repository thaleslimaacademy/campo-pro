...
export default function TurmaDetalhes() {
  ...
  async function carregar() {
    ...
    if (t) setTurma(t)
    const { data: comTurma } = await supabase.from('Atleta').select('id, nome, posicao, fotoUrl, turmaId').eq('turmaId', id).eq('ativo', true).order('nome')
    setAtletasTurma(comTurma || [])
    const { data: semTurma } = await supabase.from('Atleta').select('id, nome, posicao, fotoUrl, turmaId').eq('escolaId', 'escola-demo').eq('ativo', true).is('turmaId', null).order('nome')
  ...
  if (!turma) return <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center"><p>Turma nao encontrada.</p></div>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 pb-24">
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 mb-4">
        {turma.diasSemana && <p className="text-green-500 text-sm">{turma.diasSemana}{turma.horario ? ' - ' + turma.horario : ''}</p>}
      ...
      <div className="bg-gray-900 rounded-xl p-4 border border-gray-800 mb-4">
        ...
        {atletasTurma.length === 0 && <p className="text-gray-500 text-sm text-center py-4">Nenhum atleta nesta turma</p>}
              ...
              <div key={a.id} className="flex items-center justify-between bg-gray-800 rounded-xl p-3">
                <div className="flex items-center gap-3">
                  {a.fotoUrl
                    ...
                    : <div className="w-9 h-9 bg-green-900 rounded-full flex items-center justify-center text-sm font-bold text-green-400">{a.nome[0]}</div>
                  }
                  <div>
              ...
              {atletasSemTurma.map(a => (
                <div key={a.id} className="flex items-center justify-between bg-gray-800/50 rounded-xl p-3">
                  <div className="flex items-center gap-3">
                    {a.fotoUrl
                      ? <img src={a.fotoUrl} alt={a.nome} className="w-9 h-9 rounded-full object-cover" />
                      : <div className="w-9 h-9 bg-gray-700 rounded-full flex items-center justify-center text-sm font-bold text-gray-400">{a.nome[0]}</div>
                    }
      ...
      <a href={'/mensagens/nova?turmaId=' + id + '&turmaNome=' + encodeURIComponent(turma.nome)} className="w-full bg-blue-600 text-white py-3 rounded-xl font-bold text-sm text-center block mb-4">
