...
async function getEscolaIdBySlug(slug: string): Promise<string> {
...
export async function listarProdutosPublicos(slug: string) {
  const escolaId = await getEscolaIdBySlug(slug)
  const { data } = await supabaseAdmin
    .from('Produto')
...
export async function criarPedido(slug: string, p: {
  ...
  const dadosCobranca: Parameters<typeof criarCobrancaGenerica>[0] = {
    customer: cliente.id, billingType: p.metodoPagamento, dueDate,
  ...
  await supabaseAdmin.from('Pedido').insert({
    id: pedidoId, escolaId,
    compradorNome: p.compradorNome, compradorTelefone: telefone,
    compradorEndereco: p.compradorEndereco,
    itens: p.itens, valor: valorTotal,
    status: 'AGUARDANDO', tipoEntrega: p.tipoEntrega,
    asaasId: cobranca.id, metodoPagamento: p.metodoPagamento,
  })
