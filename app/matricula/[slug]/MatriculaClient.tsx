...
2.5 O atraso superior a 60 dias poderá acarretar a suspensão do ATLETA das atividades.
...
5.1 O CONTRATANTE autoriza, em caráter gratuito, irrevogável e por prazo indeterminado, o uso da imagem e nome do ATLETA pela ACADEMY para divulgação em redes sociais e materiais promocionais.
...
export default function MatriculaClient({ escolaId, escolaNome, escolaLogoUrl }: Props) {
  ...
  async function buscarCep(cep: string) {
    ...
    if (!data.erro) {
      setDados(prev => ({ ...prev, endereco: data.logradouro, bairro: data.bairro, cidade: data.localidade, estado: data.uf }))
    }
  }
  ...
  function avancarContrato(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!dados.cpf?.trim() && !dados.rg?.trim()) novosErros.push('CPF ou RG do atleta é obrigatório. Informe pelo menos um.')
    if (!dados.cpfResponsavel?.trim()) novosErros.push('CPF do responsável é obrigatório para geração de boletos.')
  ...
  async function confirmarMatricula() {
    ...
    setLoading(true)
    const { error } = await supabase.from('Matricula').insert({
      escolaId,
    ...
    if (error) { alert('Erro ao enviar matrícula: ' + error.message); setLoading(false); return }
    setEtapa('sucesso')
    setLoading(false)
  }
  ...
  if (etapa === 'contrato') {
    return (
      <div className="min-h-screen bg-gray-950 text-white p-6 pb-24">
        ...
        <button onClick={confirmarMatricula} disabled={!aceito || !assinaturaImg || !nomeAssinatura.trim() || loading} className="w-full bg-green-600 text-white py-4 rounded-xl font-bold text-lg disabled:opacity-40">
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6 pb-24">
      <div className="text-center mb-6">
        {escolaLogoUrl
          ? <img src={escolaLogoUrl} alt={escolaNome} className="h-16 mx-auto mb-2 object-contain" />
      ...
      <form onSubmit={avancarContrato} className="space-y-4">
        ...
        <div>
          <label className="text-sm text-gray-400">CPF do responsável * <span className="text-yellow-400 text-xs">(obrigatório para boletos)</span></label>
          <input name="cpfResponsavel" type="text" onChange={handleChange} className="w-full bg-gray-900 border border-gray-700 rounded-lg p-3 mt-1 text-white" placeholder="000.000.000-00" />
        </div>
        <div>
