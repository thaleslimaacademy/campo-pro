  ...
  useEffect(() => {
    async function carregar() {
      ...
      if (at?.tokenPais) {
        ...
        const qr = await QRCode.toDataURL(url, { width: 80, margin: 1 })
        setQrCodeUrl(qr)
      }
    ...
    <>
      <style>{`
        @media print {
          ...
          #carteirinha, #carteirinha * { visibility: visible; }
          #carteirinha {
            position: fixed;
            top: 0;
            left: 0;
      ...
      <div className="min-h-screen bg-gray-950 text-white p-6 pb-24 no-print">
        <div className="flex items-center gap-3 mb-6 no-print">
          <a href={'/atletas/' + atleta.id} className="text-gray-400">← Voltar</a>
          <h1 className="text-xl font-bold">🪪 Carteirinha</h1>
        </div>

        <p className="text-gray-400 text-sm mb-4 no-print text-center">
        ...
        <div className="flex gap-3 justify-center no-print">
          <button
            onClick={imprimir}
            disabled={imprimindo}
            className="bg-green-600 hover:bg-green-500 text-white px-8 py-3 rounded-xl font-bold disabled:opacity-50"
          >
            {imprimindo ? 'Preparando...' : '🖨️ Imprimir Carteirinha'}
          </button>
        </div>

        <p className="text-gray-500 text-xs text-center mt-4 no-print">
